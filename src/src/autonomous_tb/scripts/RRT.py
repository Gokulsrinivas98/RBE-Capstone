...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_ = (self.goal.row + self.start.row)/2
            y_ = (self.goal.col + self.start.col)/2

            # Compute rotation matrix from elipse to world frame - C            
            a1 = np.array([[(self.goal.row - self.start.row)/c_min],[(self.goal.col - self.start.col)/c_min]])
            mat = np.matmul(a1, np.array([[1, 0]]))
            # Single value decomposition (SVD)
            u, s, vh = np.linalg.svd(mat, full_matrices = True)
            # Diagonal array
            d_mat = np.diag([1, np.linalg.det(u)*np.linalg.det(vh)])
            rot_mat = np.matmul(np.matmul(u, d_mat), vh)

            # Compute diagonal matrix - L
            L = np.diag(np.array([c_best/2, np.sqrt(c_best**2 - c_min**2)/2]))
            
            # Cast a sample from a unit ball - x_ball
            x_ball = np.random.normal(loc = 0, scale = 0.5, size = 2) 
            # Transformation points
            p_tframe = np.matmul(np.matmul(rot_mat, L), x_ball)
            # New transformed coordinates of point
            point = [int(p_tframe[0] + x_), int(p_tframe[1] + y_)]
    ...
    def draw_map(self):
        ...
        # plt.savefig('Informed_RRT.png', dpi=500)
    ...
    def informed_RRT_star(self, n_pts=1000, neighbor_size=20):
        ...
        for i in range(n_pts):
            ...
            # Rewire
            n_node = self.extend(self.sample(0.05, c_best), 10)
            # If new node is not empty
            if n_node != None:
                # Rewire
                self.rewire(n_node, self.get_neighbors(n_node, neighbor_size))
