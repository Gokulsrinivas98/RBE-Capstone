...
class Score():
	...
	def __init__(self):
		...
		# Without obstacles
		self.FILE_PATH = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/ground_truth_costmap.pgm"
		# With obstacles
		self.FILE_PATH2 = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/obs_costmap.pgm"
		# self.FILE_PATH2 = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/case2_costmap.pgm"
		# Change detect map
		self.FILE_PATH_CHANGE = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/chang_map_case2.png"
		# Ground truth Change detect map
		self.FILE_PATH_CHANGE_gnd = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/gnd_trth_change_map.png"
		
		# Goal coordinates 
		self.goal  = (203, 275)
		# Start coordinates
		self.start_coordinates = [(114, 394), (135, 151), (184, 130), (111, 300), (207, 372)]
		self.grad_coordinates = [(135, 151), (184, 130), (111, 300), (207, 372)]
		...
		self.lin_conv_obs = self.grad_score(self.grad_coordinates, self.goal, self.FILE_PATH, self.FILE_PATH_CHANGE, self.kernel_size, self.sigma, self.max_cost)
		self.lin_conv_no_obs = self.grad_score(self.grad_coordinates, self.goal, self.FILE_PATH, self.FILE_PATH_CHANGE_gnd, self.kernel_size, self.sigma, self.max_cost)
		# Print gardient map score
		print("2D Convolution: ")
		org = np.sum(self.lin_conv_no_obs)
		new = np.sum(self.lin_conv_obs)
		print(f"New: {new}")
		print(f"Original: {org}")
		print(f"Percentage change: {100*(org-new)/org}")
	...
	def grad_score(self, locations, goal, grnd_trth_file_path, change_map_file_path, k_size, sig, m_cost):	
		...
		start_time = time.time()
		l_conv2d = signal.convolve2d(change_map_array, grad_map_cropped)
		end_time = time.time()
		print("Total time for calculating scores is %.2f" %(end_time-start_time))
		# print(np.unique(l_conv2d))
		# print(np.unique(change_map_array))
		# print(np.unique(grad_map_cropped))
		return(l_conv2d)
